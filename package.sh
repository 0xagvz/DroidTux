...
# Setup APT Repository (apt.inled.es)
if [ -d /etc/apt/sources.list.d ]; then
    echo "Configuring DroidTux APT repository..."
    echo "deb [trusted=yes] https://apt.inled.es/ /" > /etc/apt/sources.list.d/droidtux.list
    # Note: Using trusted=yes as we don't have the GPG key setup here. 
    # Ideal would be adding the GPG key as well.
fi
EOF
